...
export function buildPlanningPrompt(input: {
  ...
  const system = [
    ...
    '  • doc.generate → { title, prompt, format?, sourceDocument? }  // stores a document in the Company Brain only',
    '  • doc.pdf → { documentId | title }   // converts an EXISTING Brain document to PDF verbatim',
    '  • drive.write → { title, prompt, format? }   // saves a real file to the user’s Google Drive',
    '  • files.write → { path, content }   files.read → { path }',
    '',
    'When the user asks to "save/create a document in Drive" or "share a Google Doc",',
    'use drive.write (it lands in their Google Drive). Use doc.generate only when they',
    'want it kept inside the Company Brain, not in Drive.',
    '',
    'PDF: "format" is "markdown" (default, editable) or "pdf" (laid-out, shareable).',
    'Set format:"pdf" on doc.generate/drive.write whenever the user asks for a PDF,',
    'a report/deck/one-pager "to send", or anything to print, sign or attach.',
    '',
    'Choosing between the two document tools — this matters:',
    '  • NEW or CONDENSED content (a summary, a one-pager, a brief, a rewrite) →',
    '    doc.generate with format:"pdf". If it is about a specific existing',
    "    document, ALSO pass sourceDocument with that document's name so the",
    '    deliverable is written from it rather than from keyword search.',
    '  • The SAME content, just as a PDF (convert/export/"make it a PDF") → doc.pdf.',
    'doc.pdf reproduces the whole document verbatim — it never summarises, so never',
    'use it when the user asked for a summary or a one-pager.',
    'doc.pdf works on any document the Brain holds, including uploads and synced files.',
    '',
    "When referring to an existing document (doc.pdf, or doc.generate's",
    'sourceDocument), copy its name EXACTLY as it appears in RETRIEVED CONTEXT —',
    'the file name is matched as well as the title, so either resolves.',
    '',
    'Do NOT add speculative "check if this was already done / find the existing file"',
    'steps — you cannot browse the user’s Drive or local files, and only use',
    'files.read with a concrete path you were given. Plan the steps that DELIVER the',
    'request directly; producing a fresh, correct deliverable is always acceptable.',
...
const TYPE_HINTS: Array<{ type: ActionType; tool: string; keywords: RegExp }> = [
  ...
  // PDF is matched ahead of the generic document hint; doc.generate reads the
  // request itself and renders a PDF when one was asked for.
  { type: 'DOCUMENT_GENERATION', tool: 'doc.generate', keywords: /\bpdfs?\b/i },
  {
