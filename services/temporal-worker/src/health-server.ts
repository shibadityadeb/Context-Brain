...
export interface WorkerStatusSource {
  /** Temporal worker lifecycle state, e.g. INITIALIZED / RUNNING / STOPPED. */
  getWorkerState: () => string;
  getConnectionStatus: () => 'connected' | 'disconnected';
}

/**
 * Tiny HTTP endpoint so orchestrators (and the API's aggregate health
 * report) can observe this worker. GET /health returns 200 while the worker
 * runs, 503 otherwise.
 */
